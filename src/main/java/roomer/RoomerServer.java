...
public class RoomerServer {
    ...
    private static void seedUsers() {
        users.add(new User("alicia.park@pomona.edu",
                LocalDateTime.parse("Apr 8, 2025 5:03 PM", FORMATTER)));
        users.add(new User("bryson.young@pomona.edu",
                LocalDateTime.parse("Apr 8, 2025 6:42 PM", FORMATTER)));
        users.add(new User("carol.rivera@pomona.edu",
    ...
    static class FilterHandler implements HttpHandler {
        public void handle(HttpExchange ex) throws IOException {
            ...
            Boolean hasAC = null;
            try {
                if (p.containsKey("building"))
                    building = Building.valueOf(p.get("building"));
            } catch (Exception ignored) {
            }
            try {
                if (p.containsKey("occupancy"))
                    occupancy = Integer.parseInt(p.get("occupancy"));
            } catch (Exception ignored) {
            }
            if (p.containsKey("ac"))
                hasAC = p.get("ac").equalsIgnoreCase("true");
    ...
    static class SearchHandler implements HttpHandler {
        public void handle(HttpExchange ex) throws IOException {
            ...
            if (room == null)
                json(ex, 404, "{\"error\":\"Room not found\"}");
            else
                json(ex, 200, roomJson(room));
        }
    }
    ...
    static class PostSlotHandler implements HttpHandler {
        public void handle(HttpExchange ex) throws IOException {
            ...
            if (email == null) {
                json(ex, 400, "{\"error\":\"Missing email\"}");
                return;
            }
            User user = users.get(email);
            if (user == null) {
                json(ex, 404, "{\"error\":\"User not found\"}");
                return;
            }
            double price = 0.0;
            try {
            ...
            if (listing == null)
                json(ex, 400, "{\"error\":\"User already has an active listing\"}");
            else
                json(ex, 200, listingJson(listing));
        }
    }
    ...
    static class CheckTradeHandler implements HttpHandler {
        public void handle(HttpExchange ex) throws IOException {
            ...
            if (a == null || b == null) {
                json(ex, 400, "{\"error\":\"Missing emails\"}");
                return;
            }
    ...
    static class TradeHandler implements HttpHandler {
        public void handle(HttpExchange ex) throws IOException {
            ...
            if (ok)
                json(ex, 200, "{\"success\":true}");
            else
                json(ex, 400, "{\"success\":false,\"error\":\"Trade failed\"}");
        }
    }

    // GET /exchange/purchase?buyer=X&seller=Y — buys a draw time from the
    // marketplace
    static class PurchaseHandler implements HttpHandler {
        public void handle(HttpExchange ex) throws IOException {
            ...
            if (ok)
                ...
                json(ex, 400,
                        "{\"success\":false,\"error\":\"Purchase failed — insufficient balance or listing not found\"}");
        }
    }
}